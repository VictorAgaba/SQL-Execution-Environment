...
//
static int getMetaDataColumnIndex(struct TableMeta* tablemeta, char* columnName)
{
  ...
  panic("**INTERNAL ERROR: getMetaDataColumnIndex failed to find matching column");
...
//
void readDataIntoRS(char* filename, struct TableMeta* tablemeta, struct ResultSet* rs)
{
...
//
void execute_where(struct WHERE* where, struct TableMeta* tablemeta, struct ResultSet* rs)
{
  ...
  if (colType == COL_TYPE_INT)
  {
    ...
    for (int rowNum = rs->numRows; rowNum > 0; rowNum--)
    {
      ...
      if (!result)  // if false, then where is false, so delete this row from result:
...
//
void delete_unneeded_columns(struct SELECT* select, struct TableMeta* tablemeta, struct ResultSet* rs)
{
  for (int c = 0; c < tablemeta->numColumns; c++)
  {
    //
    ...
    {
      int position = resultset_findColumn(rs, 1 /*startPos*/, tablemeta->name, tablemeta->columns[c].name);
...
void reorder_columns(struct SELECT* select, struct ResultSet* rs)
{
  ...
  while (column != NULL)
  {
    ...
    int position = resultset_findColumn(rs, 1 /*startPos*/, column->table, column->name);
...
struct ResultSet* execute_query(struct Database* db, struct QUERY* query)
{
  ...
  for (int c = 0; c < tablemeta->numColumns; c++)
  {
    ...
    //
    int position = resultset_insertColumn(rs, c+1,
      tablemeta->name, tablemeta->columns[c].name,
